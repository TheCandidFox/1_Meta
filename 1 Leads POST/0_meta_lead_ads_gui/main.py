...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Meta Lead Ads Manager")
    ...
    def build_main_ui(self):
        ...
        for i in range(3):
            ...
            frame.bind("<Button-1>", lambda e, i=i: self.switch_stage(i))
    ...
    def load_pages(self):
        ...
        try:
            for p in get_pages():
                self.page_list.insert(tk.END, f"{p['name']} ({p['id']})")
    ...
    def select_page(self, e):
        ...
        if index:
            self.selected_page = get_pages()[index[0]]
    ...
    def load_forms(self):
        ...
        try:
            if not self.selected_page:
                return
            for f in get_forms(self.selected_page['id']):
                self.form_list.insert(tk.END, f"{f['name']} ({f['id']})")
        except Exception as e:
    ...
    def select_form(self, e):
        ...
        if index:
            self.selected_form = get_forms(self.selected_page['id'])[index[0]]
    ...
    def run_test(self):
        ...
        if not self.selected_form:
            ...
            return
        try:
            for lead in get_recent_leads(self.selected_form['id']):
